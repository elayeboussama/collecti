...
router.post("/create", async(req, res) => {
 ...
 router.post("/update", authenticateToken, async(req, res) => {
     try {
         ...
         await Organization.updateOne({ "_id": ObjectId(req.body._id) }, { $set: req.body });
...
router.get("/organization/:id", authenticateToken, async(req, res) => {
    try {
        const organization = await Organization.findOne({ _id: req.params['id'] });

        if(organization){
            
            res.status(201).send({ organization: organization, message: "Organization found" });
